...
int main(int argc, char **argv) 
{
	...
	while (1) 
	{
		...
		fprintf(stdout, "Timestamp %lu seconds\n",(unsigned long)time(NULL)/3600);
...
void *consumer(void *fileDescriptor){
	...
	while(1){
		...
		fprintf(stdout, "Timestamp at which consumer %d request is recieved:%lu seconds\n",fd-3,(unsigned long)time(NULL));
		/*gettimeofday(&tv,NULL);
	        tv.tv_sec;
		printf("Time elapsed is %ld",tv.tv_sec);*/
		//priority = param.sched_priority;	
		//printf("%d \n", priority);
		for(d=0;d<DELAY;d++){
			millisleep(20);
		...
		fprintf(stdout, "Timestamp at which consumer %d request is served:%lu seconds\n",fd-3, (unsigned long)time(NULL));
		Close(fd);
		millisleep(5000);//Five seconds of delay 
	}
